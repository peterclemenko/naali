		...
		#output.writerow(line)
	   	if counter > 15:
			...
			if str('1.0') in str(result):
				grsp='{"op": "action", "exectype": 1, "scene": null, "entity": "FreeLookCamera", "action": "GraspGesture", "params": []}'
				counter=0
				s.sendall(grsp +'\r\n')
				turn()
...
def turn():
	while True:
		...
		if result[0] == 'Up':
                    ms = '{"op": "action", "exectype": 1, "scene": null, "entity": "FreeLookCamera", "action": "ReleaseGesture", "params": []}' 
		    s.sendall(ms +'\r\n')
		    grasp()
		pt = pitch(x,y,z)
		rl = roll(x,y,z)
